...
public class player4 : MonoBehaviour
{
    ...
    public Transform player4Transforms;
    public float speed;
    public float jump;
    ...
    void Update()
    { 
        float horizontal2= Input.GetAxisRaw("Horizontal4");
        float vertical2 = Input.GetAxisRaw("Vertical4");
        ...
        if(Input.GetButton("Horizontal4")){
            
            player4RB.velocity = new Vector2(horizontal2 * speed, player4RB.velocity.y);
            if(horizontal2==1){
        ...
        if(Input.GetButton("Vertical4")&&isTouchingGround==true){
            anim2.SetInteger("state2",2);
            if(render.flipY==true){
                player4RB.velocity = new Vector2(player4RB.velocity.x, -jump);
                return;
            }
        ...
        if(!Input.GetButton("Horizontal4")&&!Input.GetButton("Vertical4")&&isdashing==false){
    ...
    public IEnumerator dash(){
        ...
        candash=false;
        isdashing=true;
        float oggravity=player4RB.gravityScale;
        player4RB.gravityScale=0.01f;
        if(isFacingRight==true){
            player4RB.velocity=new Vector2(1*dashspeed,0f);
        }
        ...
        if(player4RB.gravityScale<0){
            player4RB.gravityScale=oggravity*-1;
        }else{
            player4RB.gravityScale=oggravity;
        }
        isdashing=false;
        anim2.SetInteger("state2",1);
